...
Codmes 서버가 검증한 document를 Apple client가 SwiftUI로 렌더링한다.
...
- Workspace bearer 인증을 통과한 Apple client의 직접 저장·삭제는 사용자 명시
  조작이므로 즉시 반영한다.
...
binding과 editor metadata를 컴파일해 전달하고 macOS/iOS/iPadOS client가 이를
